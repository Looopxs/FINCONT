...
export const Sidebar: React.FC = () => {
  ...
  return (
    <aside
      className={`fixed top-0 left-0 bottom-0 z-40 bg-white border-r border-slate-200/80 flex flex-col justify-between transition-all duration-300 select-none ${
        collapsed ? "w-20" : "w-64"
      }`}
    >
      {/* Top Brand Header */}
      <div>
        <div className="h-18 px-5 border-b border-slate-100 flex items-center justify-between">
          <NexoraLogo href="/" size={collapsed ? "sm" : "md"} showText={!collapsed} />
          <button
